var idbaiviet = document.getElementById('edt_idbaiviet');
var idnguoidung = document.querySelector('.id_nguoidung');
var machude = document.getElementById('edt_machude');
var tieude = document.getElementById('edt_tieude');
var noidung = document.getElementById('edt_noidung');
var luubaiviet = document.getElementById('btn_luu_baiviet');
var tbodyBaiviet = document.getElementById('tbody_baiviet');
var inp_hinhanh = document.getElementById('input_hinhanh');
var refresh = document.getElementById('id_load_baiviet');
var hinhanhtam="a";
...
function hienThiDuLieu() {
    ...
    baiviets.forEach(function (baiviet, index) {
      ...
      dongMoi.innerHTML = `
        ...
        <td>${baiviet.idnguoidung}</td>
        <td>${baiviet.machude}</td>
        <td>${baiviet.tieude}</td>
        <td>${baiviet.noidung}</td>
        <td><img   width="50px" height="50px" src="${baiviet.hinhanh}" alt="Hình ảnh"></td>
        <td>${baiviet.thoiGianhientai}</td>
      `;
      ...
      btnSua.addEventListener('click', function (event) {
        ...
        idbaiviet.value = baiviet.idbaiviet;
        idnguoidung.value = baiviet.idnguoidung;
        machude.value = baiviet.machude;
        tieude.value = baiviet.tieude;
        noidung.value = baiviet.noidung;
        hinhanhtam = baiviet.hinhanh;
 ...
 btnxoa.addEventListener('click', function (event) {
    ...
    var tentkToDelete = baiviet.idbaiviet;

    // Gọi hàm xóa người dùng
    xoabaiviet(tentkToDelete);
});
      ...
      tbodyBaiviet.appendChild(dongMoi);
    });
  }
...
luubaiviet.addEventListener('click', function () {
    var idbaiviet_Value = idbaiviet.value;
    var idnguoidung_Value = idnguoidung.textContent;;
    ...
    if (inp_hinhanh.files.length > 0) {
        fr.readAsDataURL(inp_hinhanh.files[0]);
    ...
    function updateUserInfo(hinhAnh) {
        ...
        var index = baiviets.findIndex(function (baiviet) {
            return baiviet.idbaiviet == parseInt(idbaiviet_Value);
        });
...
function xoabaiviet(idbaiviet) {
    // Hiển thị hộp thoại xác nhận
    var xacNhan2 = confirm('Bạn có chắc chắn muốn xóa bài viết không?');
    ...
    if (xacNhan2) {
        ...
        // Tìm vị trí của người dùng có tên đăng nhập cụ thể
        var index = baiviets.findIndex(function (baiviet) {
            return baiviet.idbaiviet === idbaiviet;
        });
        ...
        if (index !== -1) {
            ...
            alert('Tên bài viết không tồn tại.');
        }
    }
}
